...
- Updated the Copilot instruction template for purpose-built task modules to register exported `Task.*` aliases in an always-enabled `prefix.ps1` instead of alias-only files under `Public`, preventing ModuleBuilder from adding task alias names to `FunctionsToExport`.
